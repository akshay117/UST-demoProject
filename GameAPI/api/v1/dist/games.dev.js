...
module.exports = function (router) {
  ...
  router.route(URI).post(function (req, res, next) {
    console.log("POST  Games"); //1. Get the data

    var doc = req.body; //2. Call the insert method
...
var processMongooseErrors = function processMongooseErrors(message, method, endpoint, err, payload) {
  var errorList = []; // Check for validation error
...
var processValidationErrors = function processValidationErrors(err) {
  var errorList = []; // check name
