...
var data = require('../data/games')

// Save a single game
db.save(data.SingleRow,(err, saved)=>{
    if(err){
...
db.saveMany(data.MultipleRows,(err, docs)=> {
    if(err){
...
var selectCriteria = {released : {$lte : new Date()}}

db.select(selectCriteria, (err, data)=>{
    if(err){
...
// Update Operation
var updateCriteria = {name:"CyberPunk 2077"}
